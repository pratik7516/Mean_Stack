...
let jsonDomOperation = function (refjson) {
    ...
    for (let i = 0; i < refjson.length; i++) {                      //Approach_1
        let item = refjson[i];
 ...
 * XML HANDLER
 */
let callAjaxXML = function () {                                                                                 //By Json Handler
    let xhr = new XMLHttpRequest();
...
let xmlDomHandlerV1 = (xmlDoc) => {                                             //Another Approach of Dom Operation
...
let xmlDomHandler = (xmlDoc) => {
    const authorList = xmlDoc.querySelectorAll("Author");                   //Another Approach
